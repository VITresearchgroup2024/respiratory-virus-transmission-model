...
X_train, X_test, y_train, y_test, idx_train, idx_test = train_test_split(
    X_protein, y, df.index, test_size=0.2, random_state=42, stratify=df["strat_key"]
)

# Create k-mer "documents" for Protein
X_train_docs = [" ".join(get_kmers(seq, 3)) for seq in X_train] # change kmer size as requirement
X_test_docs  = [" ".join(get_kmers(seq, 3)) for seq in X_test]
...
print(f"Protein Only - Best Params: {grid.best_params_}, Best CV ROC-AUC: {grid.best_score_:.4f}")

# Evaluation
y_prob = best_model.predict_proba(X_test_sel)[:,1]
y_pred = (y_prob >= 0.5).astype(int)
print("\nProtein Only - Classification Report:\n", classification_report(y_test, y_pred))
print("Protein Only - ROC-AUC Score:", roc_auc_score(y_test, y_prob))
cm = confusion_matrix(y_test, y_pred)
print(f"Protein Only - Confusion Matrix:\nTN: {cm[0,0]}, FP: {cm[0,1]}\nFN: {cm[1,0]}, TP: {cm[1,1]}")
